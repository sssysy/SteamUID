...
async def steamasf_start_farming(bot: Bot, ev: Event):
    ...
    try:
        ...
        # 轮询等待 ASF 刷新并抓取 CardsFarmer 进度数据（通常需 1~3 秒）
        final_data = None
        for _ in range(4):
            await asyncio.sleep(1.0)
            ...
            if updated_bot_info:
                ...
                final_data = data
                if data["total_games_count"] > 0:
                    break

        if final_data and final_data["total_games_count"] > 0:
            total_games = final_data["total_games_count"]
            total_cards = final_data["total_cards_count"]
            time_formatted = final_data["formatted_time"]
            current_games = final_data["current_games"]
            ...
            # 确认无卡可挂，自动暂停挂卡并通知用户
            await ASFClient.pause_farming(bot_name)
            await bot.send("当前暂无需要挂卡的游戏，所有卡片已全部收集完毕，已自动暂停挂卡。")
            return
