...
| 命令 | 说明 |
|------|:------:|
| `steam绑定` | 使用 OpenID 或附带好友码 绑定 steam |
| `steam asf绑定` | 使用 ASF 方式登录绑定 steam |
| `steam解绑` | 使用 OpenID 或附带好友码 解绑 steam |
| `steam asf解绑` | 删除当前账号的 ASF 登录凭证 |
...
| `steam订阅降价` | 订阅商店游戏降价信息 |
| `steam取消订阅降价` | 取消订阅商店游戏降价信息 |
...
| 命令 | 说明 |
|------|:------:|
| `steam我的排行` | 查看个人群内游玩时长排行榜与统计 |
| `steam群玩家排行` | 查看群玩家总时长排行榜 |
| `steam群游戏排行` | 查看群游玩时长排行榜 |
| `steam群游戏玩家排行` | 查看指定游戏在本群的玩家游玩时长排行榜 |
...
| `steam年度回顾` | 查看指定steam的年度回顾分享图 |
| `steam群友状态` | 查看本群已绑定群友的实时在线与游戏状态 |

#### ASF 相关
| 命令 | 说明 |
|------|:------:|
| `steam探索队列` | 调用 ASF 自动完成 Steam 探索队列并返回游戏列表 |
| `steam开启自动探索` | 开启当前账号每日自动探索队列任务 |
| `steam关闭自动探索` | 关闭当前账号每日自动探索队列任务 |
| `steam自动探索状态` | 查看绑定账号的每日自动探索队列开关状态 |
| `steam开启挂卡` | 启动或恢复 ASF 自动挂卡任务 |
| `steam结束挂卡` | 停止或暂停 ASF 自动挂卡任务 |
| `steam挂卡状态` | 查询当前剩余挂卡游戏数量与预计时间 |
